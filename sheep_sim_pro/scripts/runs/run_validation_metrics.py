...
OUT_ROOT    = HERE / "outputs" / "chapter3_validation"
SCENARIO    = "abundant"
LANDSCAPES  = [7, 23, 41]
STEPS       = 1920
N_SHEEP     = 40
...
def study_landscape(landscape: int) -> dict:
    ...
    for S in range(1, MAX_SEEDS + 1):
        ...
        renamed = L_dir / f"metrics_abundant_L{landscape:02d}_S{S:02d}.csv"
...
def write_summary_md(summaries: list[dict], out_path: Path) -> None:
    ...
    lines = []
    lines.append("# Validation metrics — abundant scenario\n")
...
def main() -> None:
    ...
    print("=" * 72, flush=True)
    print(f"Validation metrics — abundant × {LANDSCAPES}", flush=True)
