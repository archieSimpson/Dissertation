...
class FoodLandscape:
    ...
    def sample_health(self, x: float, y: float, field: FieldConfig) -> float:
        row, col = self._index(x, y, field)
        return float(self.health[row, col])

    def sample_quality(self, x: float, y: float, field: FieldConfig) -> float:
        row, col = self._index(x, y, field)
        return float(self.biomass[row, col] * self.health[row, col])
    ...
    def sensory_gradient(self, x: float, y: float, field: FieldConfig, radius: float) -> np.ndarray:
        ...
        left = self.local_quality(clamp(x - eps_x, 0.0, field.width), y, radius, field)
        right = self.local_quality(clamp(x + eps_x, 0.0, field.width), y, radius, field)
        down = self.local_quality(x, clamp(y - eps_y, 0.0, field.height), radius, field)
        up = self.local_quality(x, clamp(y + eps_y, 0.0, field.height), radius, field)
...
def build_food_landscape(rng: np.random.Generator, field: FieldConfig, food_cfg: FoodConfig, scenario: str) -> FoodLandscape:
    ...
    return FoodLandscape(biomass=biomass, carrying_capacity=carrying_capacity, ndvi=ndvi, health=health, grazing_pressure=grazing_pressure, x_coords=xs, y_coords=ys)
