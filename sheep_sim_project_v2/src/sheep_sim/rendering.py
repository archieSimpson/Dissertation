...
class LiveRenderer:
    def __init__(
        ...
        gs = GridSpec(2, 3, figure=self.fig, width_ratios=[3.5, 0.15, 1.45], height_ratios=[1.0, 1.0])
    ...
    def draw(self, flock: list[SheepAgent], step: int, scenario: str) -> None:
        ...
        im = self.ax_field.imshow(
            ...
            alpha=0.88,
            cmap="YlGn",
            vmin=float(np.min(food_img)),
            vmax=float(np.max(food_img) + 1e-9),
        )
        ...
        self.ax_field.contour(xs, ys, self.env.terrain, levels=terrain_contours, linewidths=0.6, alpha=0.30, colors="black")
    ...
    def _update_colorbar(self, image) -> None:
        ...
        cbar.set_label("Food density", rotation=90)
    ...
    def _draw_side_panel(self, counts: Counter, mean_speed: float, spread: float, pol: float, scenario: str, step: int) -> None:
        ...
        for state in BehaviourState:
            ...
            self.ax_panel.text(0.08, y, f"{STATE_LABELS[state]}: {STATE_DESCRIPTIONS[state]}", fontsize=9.8, va="center")
            y -= line

        y -= 0.02
        self.ax_panel.text(0.0, y, "Runtime summary", fontsize=13, fontweight="bold", va="top")
        y -= line
        self.ax_panel.text(0.0, y, f"Scenario: {scenario}", fontsize=10.5, va="center")
        y -= line * 0.85
        self.ax_panel.text(0.0, y, f"Step: {step}", fontsize=10.5, va="center")
        y -= line * 0.85
        self.ax_panel.text(0.0, y, f"Mean speed: {mean_speed:.2f} field units / step", fontsize=10.5, va="center")
        y -= line * 0.85
        self.ax_panel.text(0.0, y, f"Flock spread: {spread:.2f}", fontsize=10.5, va="center")
        y -= line * 0.85
        self.ax_panel.text(0.0, y, f"Polarization: {pol:.2f}", fontsize=10.5, va="center")
        y -= line * 0.85
        self.ax_panel.text(0.0, y, f"Animation speed: ~{speedup:.0f}x real time", fontsize=10.5, va="center")
        ...
        for state in BehaviourState:
            ...
            frac = count / total
            self.ax_panel.add_patch(mpatches.Rectangle((0.0, y - 0.02), 0.65 * frac, 0.028, facecolor=STATE_COLOURS[state], edgecolor="none"))
            self.ax_panel.add_patch(mpatches.Rectangle((0.0, y - 0.02), 0.65, 0.028, facecolor="none", edgecolor="#999999", linewidth=0.8))
            self.ax_panel.text(0.69, y - 0.005, f"{STATE_LABELS[state]}: {count}", fontsize=10, va="center")
            y -= line
        ...
        y -= line
        notes = [
            "Green clusters settling on patches indicate local grazing residence.",
            "Blue, straighter motion indicates active travel or search.",
            "Orange spikes usually mark social pull and regrouping.",
            "Grey patches indicate circadian inactivity or local rest.",
            "Dashed white circle approximates current flock spread around centroid.",
        ]
        for note in notes:
            self.ax_panel.text(0.0, y, f"• {note}", fontsize=9.7, va="top", wrap=True)
            y -= 0.075
...
def save_final_frame(
    ...
    field_img = 0.7 * food.values + 0.3 * env.shade
    ax.imshow(
        field_img,
        origin="lower",
